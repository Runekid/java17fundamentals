...
public class BackEndDataProvider extends VerticalLayout {
    ...
    public BackEndDataProvider() {
        ...
        ConfigurableFilterDataProvider<Person, Void, AgeGroup> filteredDataProvider = dataProvider.withConfigurableFilter();
        grid.setDataProvider(filteredDataProvider);
        // TODO add value change listener to filter and update the DataProvider
        // accordingly
        filter.addValueChangeListener(selectedAgeGroup -> filteredDataProvider.setFilter(selectedAgeGroup.getValue()));
